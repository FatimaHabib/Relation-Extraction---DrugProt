...
class ChemProtDataset(Dataset):
  def __init__(self, sentences,abs_id,arg1,arg2,e1,e2,pos1,pos2,e1_type1,e2_type, labels, tokenizer, max_len):
  ...
  def __getitem__(self, item):
    ...
    d2 = find_pos(sentence,int(float(pos2)))
    d1 = torch.Tensor(d1)
    d2 = torch.Tensor(d2)
    print("Iam in get item d1 ",type(d1))
    print("d2 type",type(d2))
    print("e1 type",type(e1),type(e2))
